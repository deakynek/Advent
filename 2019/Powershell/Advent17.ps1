...
$file = Get-Content Advent17_input.txt
$EngineArray = @(1)
$EngineIndex = 0
$EngineSoftwares = @{}
$Output = 1
...
$DirectionInput = @()
$Master = "A,A,C,B,C,B,C,B,C,A"
$A = "R,10,L,12,R,6"
$B = "R,10,L,12,L,12"
$C = "R,6,R,10,R,12,R,6"
$Feedback = "n"

$dir = @($Master,$A,$B,$C,$Feedback)
foreach($comInput in $dir)
{
	for($i =0; $i -lt $comInput.length; $i++)
	{
		$DirectionInput += [int][char]$comInput.Substring($i,1)
	}
	$DirectionInput += 10
}

Write-Host $DirectionInput
$DirIndex = 0
($EngineSoftwares[$EngineArray[0]]["Input"]) = $DirectionInput[0]
...
do
{
	...
	{
		$DirIndex++
		if($DirIndex -lt $DirectionInput.count)
		{
			$EngineSoftwares[$engine]["Input"] = ($DirectionInput[$DirIndex])
			$EngineSoftwares[$engine]["InputConsumed"] = $false
		}
		else
		{
			$EngineSoftwares[$engine]["Input"] = $null
		}
	...
	if($EngineSoftwares[$engine]["executeNextEngine"])
	{
		...
		{
			SetPoint -Pos $currentPos -marker $outInt -color "white" -steps 0
			$currentPos[0]++
		}
	}
...
Write-Host "Output was : " ($EngineSoftwares[$engine]["Output"])
Write-Host "Total Alignment Param:  $alignmentParams"
