...
<#Part 1#>
$Planets1 = @(@{"Position"= @(0,0,0); "Velocity" = @(0,0,0)}, `
...
for($i= 0; $i -lt $file.Count; $i++)
{
	...
	$Planets1[$i]["Position"][0] = [int]($Parse[1])
	$Planets1[$i]["Position"][1] = [int]($Parse[3])
	$Planets1[$i]["Position"][2] = [int]($Parse[5])	
...
{
	<#Change vel#>
	for($i = 0; $i -lt $Planets1.Count; $i++)
	{
		for($j = 0; $j -lt $Planets1.Count; $j++)
		{
			if($i -eq $j)
			{
				continue
			...
			$PlanetPos1 = $Planets1[$i]["Position"]
			$PlanetPos2 = $Planets1[$j]["Position"]
			
			if($PlanetPos1[0] -lt $PlanetPos2[0])
			{
				$Planets1[$i]["Velocity"][0] += 1
			}
	...
	for($i = 0; $i -lt $Planets1.Count; $i++)
	{
		$PlanetVel = $Planets1[$i]["Velocity"]
		
		$Planets1[$i]["Position"][0] += $PlanetVel[0]
		$Planets1[$i]["Position"][1] += $PlanetVel[1]
		$Planets1[$i]["Position"][2] += $PlanetVel[2]
	}
...
while(!($xmatch  -and $ymatch -and $zmatch))
{
	...
	for($i= 0; $i -lt $file.Count; $i++)
	{
		if($InitialState[$i]["Velocity"][0] -ne $Planets2[$i]["Velocity"][0] -or 
			$InitialState[$i]["Position"][0] -ne $Planets2[$i]["Position"][0])
		{
			$thisXMatch = $false
		}
		
		if($InitialState[$i]["Velocity"][1] -ne $Planets2[$i]["Velocity"][1] -or `
			$InitialState[$i]["Position"][1] -ne $Planets2[$i]["Position"][1])
		{
			$thisYMatch = $false
		}
		
		if($InitialState[$i]["Velocity"][2] -ne $Planets2[$i]["Velocity"][2] -or `
			$InitialState[$i]["Position"][2] -ne $Planets2[$i]["Position"][2])
		{
			$thisZMatch = $false
		}		
	}
	...
	{
		Write-Host "X Match at " $steps
		$xsteps = $steps
	}
	if(!$ymatch -and $thisYMatch)
	{
		Write-Host "Y Match at " $steps
		$ysteps = $steps
	}
	if(!$zmatch -and $thisZMatch)
	{
		Write-Host "Z Match at " $steps
		$zsteps = $steps
	}
